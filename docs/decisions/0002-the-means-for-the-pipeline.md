...
holds itself to, and not the habits of whatever was built last?
...
The four questions the choice was checked against, answered here and never
left implied.
...
requires the algebra library versions in the provenance of a run and not
...
0009 are the same runner with different markers, not two tools, and the
...
SymPy in the same process, not a subprocess to a general system, because
...
Julia with Symbolics.jl was rejected on the tensor parts, not on speed.
...
The interpreter lock is accepted, not argued away. Nothing in the covered
...
surfaces this record names, since those are held to an interface instead of
being a language the tree carries.

## Reopened when

The benchmark in issue #66 reports that the time is spread across the Python
layer instead of concentrated in canonicalisation, which would mean the seam
